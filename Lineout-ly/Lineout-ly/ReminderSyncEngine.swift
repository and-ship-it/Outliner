...
final class ReminderSyncEngine {
    ...
    func syncExternalChanges() async {
        ...
        for reminder in reminders {
            ...
            // Check for recurring reminder: recycle a completed node with the same title
            // instead of creating a duplicate. When a recurring reminder is completed,
            // Apple creates a new occurrence with a new ID — without this check,
            // each occurrence would create a duplicate node.
                ...
                if let existingNode = syncedNodes.first(where: {
                    ...
                    if let h = reminder.dueDateComponents?.hour, h != Int(NSNotFound),
                       let m = reminder.dueDateComponents?.minute, m != Int(NSNotFound) {
                        existingNode.reminderTimeHour = h
                        existingNode.reminderTimeMinute = m
                    }

                    // Move to correct date node if needed
                    let currentDate = DateStructureManager.shared.inferredDueDate(for: existingNode)
                    let cal = Calendar.current
            ...
            if !reminderTitle.isEmpty,
               let existingUnderDate = dateNode.children.first(where: {
                ...
                if let h = reminder.dueDateComponents?.hour, h != Int(NSNotFound),
                   let m = reminder.dueDateComponents?.minute, m != Int(NSNotFound) {
                    existingUnderDate.reminderTimeHour = h
            ...
            for child in dateNode.children {
                ...
                if let existing = seenTitles[title] {
                    ...
                    if child.isTaskCompleted && !existing.isTaskCompleted {
                        ...
                        // Same state — keep whichever has the more recent reminderIdentifier
                        duplicatesToRemove.append(child)
                    }
                } else {
                    seenTitles[title] = child
    ...
    // MARK: - Recurrence Sync

    /// Sync recurrence info from a reminder inbound to the node's metadata children.
    /// Creates/updates a "recurrence" child showing e.g. "Daily", "Weekly", "Every 2 weeks".
    /// Returns true if any change was made.
    @discardableResult
    private func syncRecurrenceInbound(from reminder: EKReminder, to node: OutlineNode) -> Bool {
        let existingChild = node.children.first { $0.reminderChildType == "recurrence" }
        let rules = reminder.recurrenceRules ?? []
        let hasRecurrence = !rules.isEmpty

        if hasRecurrence, let rule = rules.first {
            let description = recurrenceDescription(from: rule)
            if let child = existingChild {
                ...
                let recurrenceNode = OutlineNode(title: description)
                recurrenceNode.reminderChildType = "recurrence"
                // Insert after other metadata children (note, link), before user children
                let insertIndex = node.children.firstIndex(where: { $0.reminderChildType == nil }) ?? node.children.count
                node.addChild(recurrenceNode, at: insertIndex)
                return true
            }
        } else if let child = existingChild {
            // Recurrence removed externally
            child.removeFromParent()
            return true
        }
    ...
    /// Convert an EKRecurrenceRule to a human-readable string (e.g., "Daily", "Weekly")
    private func recurrenceDescription(from rule: EKRecurrenceRule) -> String {
